/*-- File version 0.0.0.14 from 2014.04.07 --*/
...
ko.bindingHandlers.tabs = {
    init: function (element, valueAccessor, allBindingsAccessor, viewModel) {
        ...
        var i = ko.utils.unwrapObservable(value);
        
        element.tabs({
            active: Number.isInteger(i) ? i : 0,
            activate: function (event, ui) {
                var index = element.tabs("option", "active");
                if (ko.isObs(value)) {
                    value(index);
                }

                ejs.cif(onchange, { index: index, event: event, ui: ui, context: context });
            }
        });
    },
    update: function (element, valueAccessor, allBindingsAccessor, viewModel) {
...
ko.bindingHandlers.simpleAutocomplete = {
    init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
        ...
        var autocompleteChange = allBindingsAccessor().autocompleteChange;
        var onChange = function (event, ui) {
            if (autocompleteChange) {
                var fn = ko.utils.unwrapObservable(autocompleteChange);
                fn(event, ui);
            }
