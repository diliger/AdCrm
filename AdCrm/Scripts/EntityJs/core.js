/*-- File version 0.0.0.27 from 2013.07.03 --*/
var ejs = {};
...
ejs.isEmpty = function (v, dv) {
    ...
    return typeof v == "undefined" || v === null || (isNaN(v) && !v) || v === "" || v === undefined;
};
...
ejs.onbeforeunload = function (e, a) {
    if (top.ejsSettings.allowDataLoss) {
        return;
    }

    var messages = top.onbeforeunloadEvent.raise();
...
if (!top.onbeforeunloadEvent) {
    top.onbeforeunloadEvent = ejs.createEvent();

    if ($) {
        $(top).unbind("beforeunload.ejs");
        $(top).bind("beforeunload.ejs", ejs.onbeforeunload);
        $(function () {
            $(top).unbind("beforeunload.ejs");
            $(top).bind("beforeunload.ejs", ejs.onbeforeunload);
        });
    } else {
        top.onbeforeunload = ejs.onbeforeunload;
    }
}

ejs.confirm = function (title, text, callback, details) {
...
ejs.confirmRemove = function (title, text, callback, details, options) {
    var html = ["<div><div class='alert alert-error'><i class='icon-question-sign'></i> <strong>", text, "</strong></div>"];
    ...
    var okText = "Да, точно удалить";

    if (options && options.cancelButton) {
        cancelText = options.cancelButton;
    }
    if (options && options.okButton) {
        okText = options.okButton;
    }
