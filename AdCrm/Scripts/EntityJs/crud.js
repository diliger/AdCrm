/*-- File version 0.0.2.26 from 2015.01.22 --*/
ejs.crud = function (options) {
    ...
    me.saveFilter = function (key) {
        if (!localStorage) {
            return;
        }
        key = key || host.p || "";
        key += "." + crudName + "." + filterName;
        var filter = wrapper[filterName];
        var str = ko.toJSON(filter);
        //for (var f in filter) {
        //    var item = filter[f];
    ...
    me.loadFilter = function (key) {
        ...
        for (var f in filter) {
            var item = filter[f];
            var value = data[f];
            if (!item.value || !item.condition || typeof value == "undefined" || typeof value.value == "undefined" || typeof value.condition == "undefined")
                continue;
...
ejs.crud.pureRenderer = function (options) {
    ...
    me.renderThead = function () {
        ...
        showColumns.forEach(function (it, i) {
            var print = typeof it.print != "undefined" ? it.print : "";
            html.push("<th colname='", it.name, "' title='", it.title, "' print='", print, "'");
            if (it.orderBy) {
                html.push(" orderBy='", it.orderBy, "'");
            }
            html.push("><span class='th'>", it.title, "</span></th>");
        });
        html.push("<th colname='Save'><span class='th'><a href='javascript:' class='icon fa' title='", options.textProvider.saveColumns, "'");
        html.push(" data-bind='click: ", rootPath, ".", gridName, ".saveSettings.save, visible: !", rootPath, ".", gridName, ".saveSettings.inProgress()'><i class='fa fa-floppy-o'></i></a>");
        html.push("<span class='loading icon' data-bind='visible: ", rootPath, ".", gridName, ".saveSettings.inProgress()'></span>");

        if (options.gridFilter) {
            html.push("&nbsp;<a href='javascript:' class='icon fa' title='", options.textProvider.toggleFilter, "' data-bind='click: function() { ", rootPath, ".", gridName, ".toggleFilter(); }'><i class='fa fa-filter'></i></a>");//&nbsp;
        }
        ...
        if (options.gridFilter) {
            ...
            html.push("<td colname='Save'><div class='th'><a href='javascript:' class='icon fa' data-bind='click: $root.", options.names.crudName, ".applyFilter' title='", options.textProvider.applyFilter, "'><i class='fa fa-check-circle'></i></a>");
            html.push("&nbsp;<a href='javascript:' class='icon fa' data-bind='click: $root.", options.names.crudName, ".cancelFilter' title='", options.textProvider.clearFilter, "'><i class='fa fa-times'></i></a></div></td>");
        }
    ...
    me.renderScript = function () {
        ...
        if (options.selectMany) {
            html.push("<td colname='Select' data-bind='", (options.tdStyle ? "style: {" + options.tdStyle + "}" : ""), "'><div class='td'><input type='checkbox' data-bind='value: $data.id().toString(), checked: ", rootPath, ".", options.names.selectManyName, "'/></div></td>");
        }
        showColumns.forEach(function (it, i) {
            ...
            html.push("<td colname='", it.name, "' data-bind='", (tdcss ? "css: {" + tdcss + "}, " : ""), (tdStyle ? "style: {" + tdStyle + "}" : ""), "'><div class='td ", align, "' data-bind='", dataBind, "'>");
            if (it.showTemplate) {
        ...
        html.push("<td colname='Save' data-bind='", (options.tdStyle ? "style: {" + options.tdStyle + "}" : ""), "'><div class='td'>");
        if (options.tdSaveTemplate) {
            ...
            if (options.edit) {
                ...
                html.push("title='", options.disabled ? options.textProvider.viewRow : options.textProvider.editRow, "' class='icon fa");
                ...
                html.push("'><i class='fa fa-pencil'></i></a>");
            }
            if (options.remove) {
                html.push("&nbsp;<a href='javascript:' title='", options.textProvider.removeRow, "' class='icon fa' data-bind='click: ", rootPath, ".", options.names.fnRemoveName, "'><i class='fa fa-times'></i></a>");
            }
        }
